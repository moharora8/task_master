...
- Check whether test.db is created or not in parent folder flask_app/. If not created, something went wrong with the above steps and you need to restry above steps.
...
- You can run different sql queries on the data now e.g:
  ...
  SELECT * FROM todo;
  UPDATE todo SET content = 'Buy groceries' WHERE id = 1;
